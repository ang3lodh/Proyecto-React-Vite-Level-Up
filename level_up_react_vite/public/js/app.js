...
function aviso(msg) {
  // Usar SweetAlert2 para notificaciones más visibles
  if (window.Swal) {
    Swal.fire({
      title: '¡Agregado!',
      text: msg,
      ...
      color: '#fff',
      customclassName: {
        popup: 'swal-wide' // Para hacerlo más ancho si es necesario
      }
    });
  } else {
    // Fallback al toast original si Swal no está disponible
...
function agregarAlCarritoPorId(id, cantidad = 1) {
  ...
  else carrito.push({ id: prod.id, nombre: prod.nombre, precio: prod.precio, imagen: prod.imagen, qty: cantidad, categoria: prod.categoria, codigo: prod.codigo });
  guardarCarrito();
...
function cambiarCantidad(id, delta) {
  ...
  guardarCarrito();
  if (location.pathname.includes('carrito.html')) renderizarCarrito(); // Re-render si en carrito
}
...
function renderizarProductosFiltrados() {
  ...
  productosFiltrados.forEach(p => {
    ...
    div.innerHTML = `
      <img class="imagen-producto-inicio" src="${p.imagen || 'img/logo-level-Up.png'}" alt="${p.nombre}" loading="lazy" onerror="this.src='img/logo-level-Up.png'">
      <div class="info-producto-inicio">
        <h5 class="nombre-producto-inicio">${p.nombre}</h5>
        <p class="prod-categoria">${p.categoria}</p>
...
function renderizarCarrito() {
  ...
  if (!carrito.length) {
    container.innerHTML = '<p style="color: #fff; text-align: center; padding: 50px; font-size: 1.2rem;">Tu carrito está vacío. <a href="productos.html" style="color: #28a745; text-decoration: none;">Agrega productos</a>.</p>';
    ...
    if (!usuarioActual) {
      resumen.innerHTML = `
        <div className="mensaje-login-carrito" style="background-color: #f8d7da; color: #721c24; padding: 15px; border-radius: 8px; text-align: center;">
          <p style="margin: 0; font-weight: bold;">Para proceder al pago, debes <a href="login.html" style="color: #28a745;">iniciar sesión</a> o <a href="registro.html" style="color: #28a745;">registrarte</a>.</p>
...
function configurarRegistro() {
  ...
  if (!form) {
    console.log('ERROR: No se encontró #form-registro en registro.html');
    return;
  }
...
function configurarLogin() {
  ...
  if (!form) {
    console.log('ERROR: No se encontró #form-login en login.html');
    return;
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.addEventListener('click', (e) => {
    ...
    if (ver && ver.dataset.id) {
      ...
      location.href = `detalle-producto.html?id=${encodeURIComponent(ver.dataset.id)}`;
    }
  });
});
...
function renderizarProductosAdmin() {
  ...
  PRODUCTOS.forEach(p => {
    ...
    row.innerHTML = `
      ...
      <td className="d-none d-md-table-cell">${p.descripcion.substring(0, 40)}...</td>
      <td><span className="badge bg-secondary">${p.categoria}</span></td>
      <td className="text-end">${formatearCLP(p.precio)}</td>
      <td className="text-center">
        <button className="btn btn-sm btn-outline-warning me-1" onclick="editarProducto(${p.id})">
...
function guardarProductoAdmin(e) {
  ...
  if (idx >= 0) {
    ...
    if (idxCodigo >= 0) {
      ...
      Swal.fire('Actualizado', 'Producto actualizado (mismo código)', 'success');
    } else {
...
function renderizarUsuariosAdmin() {
  ...
  usuarios.forEach(u => {
    ...
    row.innerHTML = `
      ...
      <td className="d-none d-md-table-cell">${u.correo || u.email}</td>
      <td className="text-center"><span className="badge bg-${u.rol === 'admin' ? 'danger' : u.rol === 'vendedor' ? 'warning' : 'primary'}">${u.rol}</span></td>
      <td className="d-none d-lg-table-cell">${u.fechaRegistro ? new Date(u.fechaRegistro).toLocaleDateString() : 'N/A'}</td>
      <td className="text-center">
        <button className="btn btn-sm btn-outline-warning me-1" onclick="editarUsuario(${u.id})">
          <i className="fas fa-edit"></i>
        </button>
